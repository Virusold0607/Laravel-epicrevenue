...
*/
    public static function checkTotalCap(Campaign $campaign) {
    return dd($campaign->reports()->where('status', 2)->count() <= $campaign->cap);
    }

    public static function checkDailyCap(Campaign $campaign) {
    return dd($campaign->reports()->where('created_at', Carbon::today())->where('status', 2)->count() <= $campaign->daily_cap);
    }
}
